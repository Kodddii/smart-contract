...
(async () => {
  ...
  await signerContract.approve(walletAddressDev, 200);

  const tx1 = await signerContract.transfer(walletAddressDev, 100);
  await tx1.wait();

  // const tx1 = await signerDevContract.transferFrom(walletAddress, walletAddressDev, 100);
  // await tx1.wait();

  const balanceAfterTransfer = await signerContract.balanceOf(walletAddressDev);
