...
(async () => {
  ...
  const tokenName = await signerContract.name();
  console.log(tokenName);

  const tokenSymbol = await signerContract.symbol();
  console.log(tokenSymbol);

  const tokendecimals = await signerContract.decimals();
  console.log(tokendecimals);

  const totalSupply = await signerContract.totalSupply();
